...
- `commands/simp-review.md`, `commands/simp-audit.md`, `commands/simp-debt.md` — over-engineering review tools (adapted from ponytail, MIT). Review the diff / audit the whole repo / harvest `simp:` debt comments. After-the-fact companions to the `simp` skill, which fires at write time. `review`/`audit` are prescriptive, not just diagnostic: every finding carries `why:` (provenance — for removals, why it's safe to cut; a `delete` must establish *why* the code is orphan, never assert dead from "no callers") and `fix:` (the concrete consolidation/replacement), plus a `merge` tag for cross-location redundancy and a `delete?` tag when the orphan-trace is inconclusive. Still report-only — the author / `code-simplifier` / `simplify` applies.
...
| `PreToolUse` (Bash) | `guard-dangerous-ops.sh` | Blocks `rm -rf` of filesystem roots, `DROP DATABASE`, force-push to main, etc.; also blocks failure-injection / teardown (network manipulation, container/service stop·kill·rm·down, `pkill`, `systemctl stop`) unless `.dev-squad/staging-env` exists — command-position detection, deterministic backing for auditor Bucket D |
...
| `PostToolUse` (Write) | `check-component-reuse.sh` | Warns when a new component duplicates an existing one (Reuse-First nudge); non-blocking |
