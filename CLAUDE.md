...
| `PreToolUse` (Bash) | `guard-dangerous-ops.sh` | Blocks `rm -rf` of filesystem roots, `DROP DATABASE`, force-push to main, etc.; also blocks failure-injection (`iptables`, `docker compose stop\|kill`) unless `.dev-squad/staging-env` exists (deterministic backing for auditor Bucket D) |
